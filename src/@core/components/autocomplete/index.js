...
const Autocomplete = (props) => {
  ...
  const onKeyDown = (e) => {
    ...
    if (e.keyCode === 38 && activeSuggestion !== 0) {
      ...
      // setUserInput(filteredData[activeSuggestion][filterName]);
      setShowSuggestions(false);
    } else {
      return;
    }
  ...
  const renderGroupedSuggestion = (arr) => {
    const { filterKey, customRender, filterName, suggestions } = props;

    const renderSuggestion = (item, i) => {
      if (customRender) {
        const suggestionURL = item.path
        return (
          <li
            className={classnames("suggestion-item", {
              active: filteredData.indexOf(item) === activeSuggestion,
            })}
            key={item[filterKey] || item[filterName]}
            onClick={(e) => onSuggestionItemClick(suggestionURL, e)}
            onMouseEnter={() => {
              onSuggestionItemHover(suggestions.indexOf(item));
            }}
          >
            {item[filterKey] || item[filterName]}
          </li>
        );
  ...
  const renderUngroupedSuggestions = () => {
    const { filterKey, suggestions, customRender, suggestionLimit, filterName } = props;
        ...
        if (!customRender) {
          return (
            <li
              ...
              key={suggestion[filterKey] || suggestion[filterName]}
              onClick={(e) => onSuggestionItemClick(suggestionURL, e)}
              onMouseEnter={() =>
                onSuggestionItemHover(filteredData.indexOf(suggestion))
              }
            >
              {suggestion[filterKey] || suggestion[filterName]}
            </li>
          );
        } else if (customRender) {
          return customRender(
  ...
  const renderSuggestions = () => {
    const { filterKey, grouped, filterHeaderKey, suggestions, filterName } = props;
    // ** Checks if suggestions are grouped or not.
    if (grouped === undefined || grouped === null || !grouped) {
      return renderUngroupedSuggestions();
    } else {
      return suggestions.map((suggestion) => {
        const sortData = suggestion.filter((i) => {
          const startCondition = (
            i[filterKey]
              .toLowerCase()
              .startsWith(userInput.toLowerCase()) ||
        ...
        // sortData.slice(0, suggestion);
        if (sortData.length !== 0) {
          filteredData.push(...sortData);
          return (
            <Fragment key={sortData[0].id}>
              <li className="suggestion-item suggestion-title-wrapper">
                <h6 className="suggestion-title d-flex flex-column " style={{ gap: '4px' }}>
                  {sortData[0].name}
                  <small>{sortData[0].location}</small>
                </h6>
              </li>
  ...
  return (
    <div className="autocomplete-container" ref={container}>
      <input
        ...
        value={userInput}
        className={`autocomplete-search border-0 ${props.className ? props.className : ""
          }`}
